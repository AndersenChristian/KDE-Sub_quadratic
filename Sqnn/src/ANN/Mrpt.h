	...
	void grow(const float *data, int n_test, int k_, int trees_max = -1, int depth_max = -1,
		...
		if (depth_max <= 0 || depth_max > std::log2(n_samples)) {
			throw std::out_of_range(
					"depth_max must belong to the set {1, ... , log2(n)}.\ndepth_max = " + std::to_string(depth_max)
					+ "\nn = " + std::to_string(n_samples)
					+ "\nlog(n) = " + std::to_string(std::log2(n_samples)));
		}
...
#pragma omp parallel for
	...
	*/
	static void
	build_dense_random_matrix(Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> &dense_random_matrix,
...
#pragma omp parallel for
	...
	*/
	...
	double get_projection_time(int n_trees, int depth) const {
	...
	double get_exact_time(int n_trees_, int depth, int v) const {
		return predict_theil_sen(get_candidate_set_size(n_trees_, depth, v), beta_exact);
	}

	double get_query_time(int tree, int depth_, int v) const {
		return get_projection_time(tree, depth_)
					 + get_voting_time(tree, depth_, v)
					 + get_exact_time(tree, depth_, v);
	}
